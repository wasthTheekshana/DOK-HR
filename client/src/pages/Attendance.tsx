...
import { Calendar, MapPin, Clock, User, CheckCircle, FileText, ClipboardList, BarChart2, AlertCircle } from 'lucide-react';
...
const AttendancePage: React.FC = () => {
    ...
    const [viewMode, setViewMode] = useState<'log' | 'report'>('log');
    ...
    useEffect(() => {
        if (siteFilter) {
            viewMode === 'log' ? fetchAttendance() : fetchReport();
        }
    ...
    const completeCount = attendance.filter(a => a.IN_TIME && a.OUT_TIME).length;
    const inProgressCount = attendance.filter(a => a.IN_TIME && !a.OUT_TIME).length;
    ...
    return (
        <div className="space-y-5">
            ...
            {viewMode === 'log' && !loading && attendance.length > 0 && (
